...
class TreeTest(unittest.TestCase):
    ...
    def test_responses(self):
        ...
        df = pd.read_csv("test_data/test_data.csv", index_col=0)
    ...
    def test_default_majority(self):
        ...
        df = pd.read_csv("test_data/test_data_majority.csv", index_col=0)
        df = df.loc[~df["y"].isna()]
        ...
        self.assertEqual(
            (y_hat.round(1) == df["y_hat_exp"].round(1)).sum(),
            len(df),
    ...
    def test_default_mia(self):
        ...
        df = pd.read_csv("test_data/test_data_mia.csv", index_col=0)
        df = df.loc[~df["y"].isna()]
        ...
        self.assertEqual(
            (y_hat.round(1) == df["y"].round(1)).sum(),
            len(df),
            msg="Reponse prediction for majority rule strategy not as expected",
        )
    ...
    def test_trinary_tree(self):
        ...
        df_train = pd.read_csv("test_data/train_data_trinary.csv", index_col=0)
        df_test = pd.read_csv("test_data/test_data_trinary.csv", index_col=0)
    ...
    def test_tree_cat(self):
        ...
        df = pd.read_csv("test_data/test_data_cat.csv", index_col=0)
        X = df.drop("y", axis=1)
        y = df["y"]
        ...
        for tree_name in tree_types:
            ...
            self.assertEqual(
                (y == y_hat).sum(),
                len(y),
                msg=f"Categorical test_data prediction not correct for {tree_name}",
            )
    ...
    def test_missing_feature_warning(self):
        ...
        for tree_name in tree_types:
            ...
            with self.assertWarns(
                MissingFeatureWarning,
                msg=f"Missing feature warning missing for {tree_name}",
            ):
    ...
    def test_redundant_feature_warning(self):
        ...
        for tree_name in tree_types:
            ...
            with self.assertWarns(
                ExtraFeatureWarning,
                msg=f"Redundant feature warning missing for {tree_name}",
            ):
                y_hat = tree.predict(X_extra)

    def test_classification(self):
        df = pd.read_csv("test_data/test_data_class.csv", index_col=0)
        X = df.drop("y", axis=1)
        y = df["y"]
    ...
    def test_classification_probabilities(self):
        df = pd.read_csv("test_data/test_data_proba.csv", index_col=0)
        ...
        for tree_name in tree_types:
            ...
            self.assertAlmostEqual(tree.right.y_prob["apple"], 5 / 6, msg=msg)
            self.assertAlmostEqual(tree.right.left.y_prob["apple"], 1, msg=msg)
            self.assertAlmostEqual(tree.right.right.y_prob["banana"], 1, msg=msg)
    ...
    def test_weighted_strat(self):
        df_train = pd.read_csv("test_data/train_data_weighted.csv", index_col=0)
        X_train = df_train.drop("y", axis=1)
        y_train = df_train["y"]

        df_test = pd.read_csv("test_data/test_data_weighted.csv", index_col=0)
        ...
        self.assertAlmostEqual(
            (y_hat == y_test).sum(),
            len(y_test),
            msg="Weighted strategy not working properly",
        )

    def test_weighted_strat_cat(self):
        df_train = pd.read_csv("test_data/train_data_weighted_cat.csv", index_col=0)
        X_train = df_train.drop("y", axis=1)
        y_train = df_train["y"]

        df_test = pd.read_csv("test_data/test_data_weighted_cat.csv", index_col=0)
        ...
        self.assertAlmostEqual(
            (y_hat == y_test).sum(),
            len(y_test),
            msg="Weighted strategy classification not working properly",
        )
        self.assertAlmostEqual(
            (y_prob_hat.round(2) == y_prob.round(2)).sum().sum(),
            np.prod(y_prob.shape),
            msg="Weighted strategy classification not working properly",
        )
